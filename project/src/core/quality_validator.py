...
class QualityValidator:
    ...
    def calculate_coherence_score(self, content: str, section_type: str) -> float:
        ...
        # Check 1 — paragraph structure (up to +0.30)
        ...
        # Check 2 — logical connectors (up to +0.20)
        logical_connectors = [
            'therefore', 'however', 'furthermore', 'additionally',
            'consequently', 'moreover', 'thus', 'hence', 'accordingly',
            'in contrast', 'as a result', 'in addition'
        ]
        connector_count = sum(1 for c in logical_connectors if c in content_lower)
        if connector_count >= 3:
            score += 0.20
        elif connector_count >= 1:
            score += 0.10

        # Check 3 — structured thinking markers (up to +0.20)
        structure_markers = [
            'first', 'second', 'third', 'finally',
            'in conclusion', 'next', 'lastly', 'in summary'
        ]
        marker_count = sum(1 for m in structure_markers if m in content_lower)
        if marker_count >= 3:
            score += 0.20
        elif marker_count >= 1:
            score += 0.10

        # Check 4 — content depth (up to +0.30)
    ...
    def calculate_groundedness_score(
        ...
        # Step 1 — keywords dictionary for each section type
        section_keywords = {
            "liability_assessment": [
                "liability", "negligence", "breach", "duty", "causation",
                "damages", "claim", "evidence", "standard", "reasonable",
                "plaintiff", "defendant", "infringement", "willful", "intent"
            ],
            "damage_calculation": [
                "damages", "compensation", "calculation", "quantum", "lost profits",
                "royalty", "price erosion", "actual damages", "statutory", "punitive",
                "economic loss", "financial impact", "million", "lost sales"
            ],
            "prior_art_analysis": [
                "prior art", "patent", "novelty", "obviousness", "claims",
                "validity", "anticipation", "disclosure", "publication",
                "freedom", "operate", "cite", "reference", "prosecution"
            ],
            "competitive_landscape": [
                "competitors", "market share", "positioning", "competitive advantage",
                "market", "industry", "player", "rival", "landscape",
                "differentiation", "licensing", "portfolio", "strategy"
            ],
            "risk_assessment": [
                "risk", "probability", "impact", "mitigation", "exposure",
                "likelihood", "threat", "vulnerability", "contingency",
                "scenario", "assessment", "potential", "consequence"
            ],
            "strategic_recommendations": [
                "recommendation", "strategy", "implementation", "action",
                "timeline", "milestone", "priority", "objective", "goal",
                "approach", "plan", "resource", "outcome", "recommend"
            ]
        }

        # Step 2 — get keywords for this section
        keywords = section_keywords.get(section_type, ["analysis", "assessment"])

        # Step 3 — keyword coverage (up to 0.30)
        keyword_hits = sum(1 for kw in keywords if kw in content_lower)
        keyword_ratio = keyword_hits / len(keywords)
        score += keyword_ratio * 0.30

        # Step 4 — reasoning indicators (up to 0.40)
        reasoning_phrases = [
            "based on", "because", "due to", "as a result",
            "therefore", "given that", "in light of",
            "evidence suggests", "analysis shows",
            "data indicates", "conclusion", "demonstrates", "established"
        ]
        reasoning_count = sum(1 for p in reasoning_phrases if p in content_lower)
        reasoning_score = min(reasoning_count / 3, 1.0)
        score += reasoning_score * 0.40

        # Step 5 — expected elements coverage (up to 0.30)
        if expected_elements:
            elements_covered = sum(1 for e in expected_elements if e in content_lower)
            elements_ratio = elements_covered / len(expected_elements)
            score += elements_ratio * 0.30
        else:
            score += 0.30
